...
export default function ShiurCard({
  ...
  useEffect(() => {
    // Progress lives in localStorage (client-only) — must be read after mount,
    // not during render. SSR + first client render both show no-progress (match).
  ...
  const handleDownload = useCallback(() => {
    // Direct link — streams via the browser's native downloader with no in-memory
    // blob. (download is honored same-origin; cross-origin CDNs open in a new tab.)
    const a = document.createElement("a");
    a.href = shiur.audioUrl;
    a.download = shiur.title.replace(/[^a-zA-Z0-9\s-]/g, "").trim().replace(/\s+/g, "-");
    a.target = "_blank";
    a.rel = "noopener";
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          {inProg && !isCurrentlyPlaying && onPlayFromBeginning && (
            <button
              ...
              className="w-8 h-8 rounded-full flex items-center justify-center text-brown/45
